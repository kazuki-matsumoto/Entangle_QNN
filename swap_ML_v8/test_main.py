...
def optimize(cost_func, theta_init, method='Nelder-Mead'):
    ...
    theta_0 = torch.tensor(theta_init, requires_grad=True, device=torch.device('cuda'))
    theta_opt = theta_0
    optimizer = torch.optim.SGD([theta_opt], lr=0.1)
    for i in range(OPTIM_STEPS):
        optimizer.zero_grad()
        
        f = cost_func(theta_opt)
        # print("f", f)
        # f = torch.tensor(f, requires_grad=True)
        # print("f", f)
        
        # f = -1 * torch.sum(theta_opt)
...
class SwapQNN:
    ...
    def U_out(self, qc, qr, thetas):
        ...
        print(thetas)
        thetas = thetas.to('cpu').detach().numpy().copy()
        print(thetas)
    ...
    def parametarized_gate(self, thetas):
        ...
        for id_qubit in [i for i in range(n_qubits) if i != 2]:
            print(id_qubit)
