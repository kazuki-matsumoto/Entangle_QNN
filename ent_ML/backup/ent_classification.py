...
class QclClassification:
    ...
    def __init__(self, nqubit, data_nqubits, class_nqubits, num_features, c_depth, num_class, fig_name_loss, num_measure=1):
    ...
    def output(self, theta):
        ...
        for x, y_mixed in zip(self.x_list, self.y_mixed_list):
            ...
            for y in y_mixed:
                ...
                innerproduct_block.append(np.array([y[-1], innerproduct]))
            
            innerproduct_block = np.array(innerproduct_block)
            
            innerproducts.append(innerproduct_block)
    ...
    def cost_func(self, theta):
        ...
        losses = [self.loss(is_correct, innerproduct) for y_item in y_innerproducts for is_correct, innerproduct in y_item]
        loss = np.sum(losses)
    ...
    def B_grad(self, theta):
        ...
        grad = [(self.output(theta_plus[i])[0][:, -1] - self.output(theta_minus[i])[0][:, -1]) / 2. for i in range(len(theta))]

        return np.array(grad)
    
    # for cost_func_grad of BFGS 
    def y_minus_t(self, theta):
        
        innerproduct_with_bool = self.output(theta)
        bool_for_label = innerproduct_with_bool[0][:, 0]        
        y_minus_t = innerproduct_with_bool[0][:, -1] - bool_for_label
    ...
    def cost_func_grad(self, theta):
        ...
        y_minus_t = self.y_minus_t(theta)
    ...
    def fit(self, x_list, y_list, y_mixed_list, maxiter, batch_size):
        ...
        # self.x_list = x_list
        # self.y_mixed_list = y_mixed_list
        ...
        self.maxiter = batch_size
        
        theta_opt = self.theta
        
        for x_list_batch, y_list_batch in self.batch_generator(x_list, y_mixed_list, batch_size):
            ...
            print(theta_opt)
            print()
            print(f"Initial value of cost function:  {self.cost_func(theta_opt):.4f}")
            print()
            print('============================================================')
