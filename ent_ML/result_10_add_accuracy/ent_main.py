...
def train_test_split_from_df(df, test_size, num_class, random_state):
    ...
    is_correct_X_train = X_train[:, -1]
    is_correct_y_test = y_test[y_test[:, -1] == True][:, -1]
    ...
    y_train_mixed = np.concatenate((y_train_mixed, is_correct_X_train[:, np.newaxis]), axis=1)
    ...
    # y_test = np.concatenate((y_test, is_correct_y_test[:, np.newaxis]), axis=1)
...
if __name__ == '__main__':
    ...
    qcl = QclClassification(N_QUBITS, 
                            DATA_NUM_QUBITS, 
                            CLASS_NUM_QUBITS, 
                            NUM_FEATCHERS, 
                            C_DEPTH, NUM_CLASS, 
    ...
    theta_init, theta_opt = qcl.fit(X_train, y_train, y_train_mixed, X_test, y_test, maxiter=MAX_ITER)
