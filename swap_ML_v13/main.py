...
class SwapQNN_pred():
    ...
    def predict(self, X_test, optimized_weight):
        ent_entropies = np.array([])
        
        for y in range(NUM_CLASS):    
            qc, qr = self.U_in_pred(X_test, y)
            qc = self.U_out_pred(qc, optimized_weight)
            # print(qc)
    ...
    def accuracy(self, X_test, y_test, optimized_weight):
        pred_dict = {}
        y_pred = np.array([])
        count = 0
        for x, y in zip(X_test, y_test):
            predicted = self.predict(X_test=x, optimized_weight=optimized_weight)
            y_pred = np.append(y_pred, predicted)
            pred_dict['ans{0} is {1}'.format(count, y)] = "pred{0} is {1}".format(count, predicted)
...
if __name__ == "__main__":
    ...
    qc_pred = SwapQNN_pred(simulator, n_qubits=4)
