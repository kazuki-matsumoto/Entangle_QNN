...
class SwapQNN:
    ...
    def U_in_pred(self, X_test, y, n_qubits):
    ...
    def predict(self, X_test, theta_opt):
        ...
        for y in range(NUM_CLASS):
            ...
            qc, qr = self.U_in_pred(X_test, y, n_qubits=4)
            qc.append(self.classify_rot_gate(n_qubits=4, state_phase=phase))
...
if __name__ == "__main__":
    ...
    for label, df_split in df_dict.items():   
        ...
        phase = state_phase(label)
        
        initial_thetas = []
        for x in X_train:
            print(label, x)
            print("phase", phase)
            qc = SwapQNN(nqubits=nqubits, simulator=simulator, nshots=nshots, state_phase=phase, X_train=x, y_train=label)
            initial_theta = qc.initial_theta()
